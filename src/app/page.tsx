    ...
    <div className="min-h-screen bg-[#FFFDF9] text-slate-800 font-sans selection:bg-red-600 selection:text-white">
      ...
      <section id="rehydration-guide" className="py-24 bg-white border-y border-red-100">
        <div className="max-w-6xl mx-auto px-6">
          ...
          >
            ...
            <div className="space-y-8 md:pl-12 md:border-l border-gray-100 h-full">
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  {(activeTab === "rehydrate" ? [
                    `Take ${coconutGrams}g Maharani Coconut Powder in a clean bowl.`,
                    `Sprinkle ${warmWaterMl} ml warm water and mix gently.`,
                    "Allow to rest for 10-15 minutes to regain natural soft texture.",
                    "Use directly in Thoran, Avial, Chutneys & Gravies."
                  ] : [
                    `Add ${coconutGrams}g powder to ${warmWaterMl} ml warm water.`,
                    "Soak for 10 mins and blend thoroughly in a mixer.",
                    "Strain through a fine sieve or muslin cloth.",
                    "Use fresh milk for stews, payasam, and rich curries."
                  ]).map((step, idx) => (
      ...
      <section id="recipes" className="py-24 bg-white border-t border-gray-100">
        <div className="max-w-7xl mx-auto px-6">
          ...
          >
            {[
              { name: "Traditional Thoran", desc: "Rehydrate and stir-fry directly with vegetables and spices." },
              { name: "Kerala Avial", desc: "Blend into a smooth paste with green chilies and cumin." },
              { name: "Coconut Stew", desc: "Extract rich coconut milk for delicious vegetable or meat stews." },
              { name: "Payasam", desc: "Add milk or rehydrated coconut for authentic sweet preparations." }
            ].map((recipe, idx) => (
              <motion.div key={idx} variants={fadeUp} className="bg-gray-50 p-8 rounded-[2rem] border border-gray-100 hover:bg-white hover:shadow-xl hover:shadow-red-900/5 hover:-translate-y-2 transition-all duration-300 group">
                <div className="text-4xl font-black text-red-100 mb-6 group-hover:text-red-200 transition-colors">
                  0{idx + 1}
                </div>
                <h4 className="font-black text-gray-900 text-xl mb-3">{recipe.name}</h4>
                <p className="text-sm text-gray-600 leading-relaxed font-medium">{recipe.desc}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
